package package_228
{
   import alternativa.osgi.OSGi;
   import package_13.Long;
   import package_38.name_69;
   import package_38.name_840;
   import package_38.name_860;
   import package_39.Model;
   import platform.client.fp10.core.model.name_66;
   
   public class ClanPermissionsModelBase extends Model
   {
       
      
      private var var_531:name_840;
      
      protected var server:name_2063;
      
      private var client:name_613;
      
      private var var_25:Long;
      
      private var var_1388:Long;
      
      private var var_1389:name_860;
      
      public function ClanPermissionsModelBase()
      {
         this.var_531 = name_840(OSGi.getInstance().getService(name_840));
         this.client = name_613(this);
         this.var_25 = Long.getLong(1602467880,168781542);
         this.var_1388 = Long.getLong(871743923,-1602562917);
         super();
         this.method_770();
      }
      
      protected function method_770() : void
      {
         this.server = new name_2063(name_66(this));
      }
      
      protected function method_771() : name_2062
      {
         return name_2062(var_19[Model.object]);
      }
      
      override public function invoke(param1:Long, param2:name_69) : void
      {
         switch(param1)
         {
            case this.var_1388:
               this.client.method_1459(this.var_1389.decode(param2) as Vector.<ClanAction>);
         }
      }
      
      override public function get id() : Long
      {
         return this.var_25;
      }
   }
}
